"""Unit tests for homeassistant.components.opnsense.binary_sensor.
...
async def test_async_setup_entry_creates_entities_when_enabled(
    ...
    """Create notices entity when notices sync is enabled."""
...
def test_pending_notices_sensor_update_paths_param(
    ...
    expect_write_called: bool,
    expect_available: bool,
    expect_is_on: bool | None,
    expect_pending: list[dict[str, int]] | list[Any] | None,
    make_config_entry: Any,
) -> None:
...
def test_pending_notices_sensor_guard_branches(make_config_entry: Any) -> None:
